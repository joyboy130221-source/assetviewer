...
function readMaximoError(value, fallback) {
    return value?.['oslc:Error']?.['oslc:message'] || value?.error?.['oslc:message'] || value?.error || value?.message || fallback;
}
...
async function maximoFetch(env, url, options = {}) {
    ...
    try {
        ...
        if (!upstream.ok) {
            const e = new Error(readMaximoError(data, `Maximo API returned ${upstream.status}`));
